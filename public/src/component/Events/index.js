...
function RenderConditions(error, events, viewDetails, deleteEvent, editEvent, orderParty) {
  switch (error) {
    ...
    case "":
      return (
        <div>
          <CreateEventRedirect/>
          <EventList viewDetails={viewDetails} orderParty={orderParty} editEvent={editEvent} events={events} deleteEvent={deleteEvent} />
        </div>
      )
    default:
      return null;
...
class EventsBase extends Component {
  ...
  viewDetails = (index) =>{
    const {events , calendarId } = this.state
      this.props.history.push(ROUTES.EVENT_DETAILS, {events: events[index] , calendarId: calendarId})
  }

  deleteEvent  = (index) =>{
      const { calendarId } = this.state
      const eventId = this.state.events[index].id
      this.props.gapi.deleteEvent(calendarId , eventId)
      .then((res)=>{
    ...
    return (
      <section id="maincontent">
        ...
        <div className="container">
          <div className="Row">
            <div className="span11">{RenderConditions(error, events, this.viewDetails, this.deleteEvent, this.editEvent, this.orderParty)}</div>
          </div>
        </div>
      </section>
    );
